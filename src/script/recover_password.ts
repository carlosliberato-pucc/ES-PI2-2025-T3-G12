...
const form = document.getElementById('recoverForm') as HTMLFormElement;
const emailInput = document.getElementById('email') as HTMLInputElement;
const recoverBtn = document.getElementById('recover-btn') as HTMLButtonElement;
const messageDiv1 = document.getElementById('message') as HTMLDivElement;

console.log('🚀 Script recover_password.ts carregado!');

function showMessage1(text: string, type: 'success' | 'error') {
...
form.addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if (data.success) {
            showMessage1(
                'Se o e-mail existir em nosso sistema, você receberá um link de recuperação. Verifique sua caixa de entrada e spam.',
                'success'
            );
            emailInput.value = '';
