...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const btnDeleteTurma = edicaoCard?.querySelector('.btn-open-delete');
    btnDeleteTurma?.addEventListener('click', async (e) => {
        ...
        try {
            ...
            if (response.ok && data.success) {
                ...
                const cardParaDeletar = document.querySelector(`.card[data-id="${turmaId}"]`);
                if (cardParaDeletar) {
    ...
    const initModalFormula = () => {
        ...
        btnGerenciar?.addEventListener('click', (e) => {
            ...
            if (turmasFormulaAtual) {
                if (selectTipo)
                    selectTipo.value = turmasFormulaAtual.tipo;
                if (inputFormula)
                    inputFormula.value = turmasFormulaAtual.formula;
            }
            else {
                if (selectTipo)
        ...
        const fecharModalFormula = () => {
            ...
            const outrosModaisAbertos = (modalComponentes?.style.display === 'block') ||
    ...
    const atualizarSidebarFormula = () => {
        ...
        if (divsFormula.length >= 2) {
            if (turmasFormulaAtual) {
                ...
                divsFormula[0].style.fontStyle = 'italic';
            }
        }
    };
    // ========================================
    ...
    const initModalComponentes = () => {
        ...
        const fecharModalComponentes = () => {
            ...
            const modalFormula = document.getElementById('modal_formula');
            const modalConfirmacao = document.getElementById('modal_confirmacao');
            const outrosModaisAbertos = (modalFormula?.style.display === 'block') ||
                (modalConfirmacao?.style.display === 'block');
            if (!outrosModaisAbertos) {
                modalOverlay?.classList.remove('ativo');
            }
